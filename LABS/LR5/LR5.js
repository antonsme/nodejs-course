...
const promptUser = () => {
    rl.question(`choose (sd sc ss)  `, (input) => {
        const command = input.trim().split(' ');
        const mainCommand = command[0];
        const param = parseInt(command[1]);
        if (mainCommand === 'sd') {
            if (isNaN(param)) {
                clearTimeout(stopFunc);
                console.log("server's stopping is rejected");
            } else {
                clearTimeout(stopFunc);
                console.log(`server will be stoped after ${param} seconds`);
                stopFunc = setTimeout(stopServer, param * 1000);
            }
        }

        if (mainCommand === 'sc') {
            if (isNaN(param)) {
                DB.stopCommitInterval()
                console.log("end of interval")
            } else {
                DB.startCommitInterval(param)
                console.log(`interval will be working every ${param} seconds`)
            }
        }

        if (mainCommand === 'ss') {
            if (isNaN(param)) {
                console.log(`info about server after ${param}`);
            } else {
                DB.saveInfoForCurrentTime(param)
                console.log(`info about server has delay ${param} seconds`)
            }
        }
        promptUser();
    });
...
const server = http.createServer((reg, res) => {
    ...
    if (parsedUrl.pathname == "/") {
        ...
        res.end(JSON.stringify(DB.getServerInfo()))
