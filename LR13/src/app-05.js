...
const server = net.createServer((socket) => {
    ...
    let sum = 0
    server.getConnections((e, clientsCounter) => {
        if (!e) {
            console.log("CONNECTED", PORT, clientsCounter)
        }
    })
    clients.set(socket, sum)
    let buf = Buffer.alloc(4)

    socket.on('data', (data) => {
        console.log(`data from client: ${socket.remoteAddress} , ${socket.remotePort}`, data, sum)
